<template>
  <div class="scroll-wrapper" :style="getScrollWrapperStyle">
    ...
    <div class="scroll" :style="getScrollStyle" ref="scroll">
      ...
      <div class="loading">
        <div v-if="!loadCompleted" class="flex-row row-center"><i class="data-loader mr8">loading</i> 加载中</div>
  ...
  export default {
    ...
    computed: {
      ...
      getRefreshLoadStyle() {
        ...
        return {
          will: refreshStage === 0,
          doing: refreshStage === 1,
          did: refreshStage === 2,
          error: refreshStage === 3,
        };
      },
      getRefreshStyle() {
        let transition = '';
    ...
    methods: {
      ...
      */
      isForbidMove() {
        return this.copyLoadStage === 1 || this.copyRefreshStage === 1;
      },
      /**
       *  获取元素滚动到底部的值
      */
...
<style lang="scss">
  ...
  .scroll-wrapper{
    position: relative;
    overflow: hidden;
    touch-action: none; // 阻止浏览器默认事件， 当触控事件发生在元素上时，不进行任何操作。
  }
